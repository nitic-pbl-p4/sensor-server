...
def recognizer(events: Dict[str, Event], state: State):
    with open('face/data/id.json') as f:
                ids = json.load(f)
    # キャプチャを開く
    ...
    # 特徴を読み込む
    dictionary = []
    files = glob.glob("face/feature/*.npy")
    for file in files:
    ...
    while not events["stop"].is_set():
        ...
        for face in faces:
            ...
            if(result):
                print("name : {} , id : {}".format(user[0], ids[user[0]]))
                state.person = Person(
                     ids[user[0]],seenAt=datetime.now(timezone.utc)
                )
            # 顔のバウンディングボックスを描画する
